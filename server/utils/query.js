...
const sqlContent = fs.readFileSync(path.resolve(__dirname,'..','./sql/lls_blog.sql'),'utf-8')
let pool
...
init.connect()
init.query('CREATE DATABASE lls_blog',err=>{
    ...
    if(err){
        console.log('lls_blog database created already')
    }else{
        console.log('create lls_blog Database')
